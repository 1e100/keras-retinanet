...
class SimpleGenerator(Generator):
    ...
    def load_annotations(self, image_index):
        annotations = {
            "labels": self.labels[image_index],
...
class TestLoadAnnotationsGroup(object):
    def test_simple(self):
        input_bboxes_group = [np.array([[0, 0, 10, 10], [150, 150, 350, 350]])]
        input_labels_group = [np.array([1, 3])]
    ...
    def test_multiple(self):
        input_bboxes_group = [
            np.array([[0, 0, 10, 10], [150, 150, 350, 350]]),
            np.array([[0, 0, 50, 50]]),
        ]
        input_labels_group = [np.array([1, 0]), np.array([3])]
        expected_bboxes_group = input_bboxes_group
        expected_labels_group = input_labels_group
...
class TestFilterAnnotations(object):
    def test_simple_filter(self):
        input_bboxes_group = [np.array([[0, 0, 10, 10], [150, 150, 50, 50]])]
        input_labels_group = [np.array([3, 1])]

        input_image = np.zeros((500, 500, 3))

        expected_bboxes_group = [np.array([[0, 0, 10, 10]])]
        expected_labels_group = [np.array([3])]
    ...
    def test_multiple_filter(self):
        input_bboxes_group = [
            ...
            np.array([[0, 0, -1, -1]]),
            np.array([[-10, -10, 0, 0], [-10, -10, -100, -100], [10, 10, 100, 100]]),
            np.array([[10, 10, 100, 100], [10, 10, 600, 600]]),
        ]
        ...
        expected_bboxes_group = [
            np.array([[0, 0, 10, 10], [150, 150, 350, 350], [1, 1, 2, 2]]),
            np.zeros((0, 4)),
    ...
    def test_complete(self):
        input_bboxes_group = [
            np.array([[0, 0, 50, 50], [150, 150, 50, 50]], dtype=float)  # invalid bbox
        ]
        ...
        assert (
            len(labels) == 1 and labels[0] == 0
        ), "Expected only class 0 to be present, but got classes {}".format(labels)
